...
Rodilla izquierda-->b(adelante),B(atrás)
Rodilla derecha-->n(adelante),N(atrás)

Brazo izquierdo-->x(adelante),X(atrás)
Brazo izquierdo-->r(arriba),R(abajo)

Brazo derecho-->c(adelante),C(atrás)
Brazo derecho-->t(arriba),T(abajo)

Cabeza-->q(derecha),Q(izquierda)
Cuello-->z(),Z()
*/
#include "texture.h"
#include "figuras.h"
...
float posX = 0, posY = 2.5, posZ = -3.5, rotRodIzq = 0; // Posición para que se  pueda mover
...
#define MAX_FRAMES 30 //Número máximo de keyframes ////////////// Poner N Keyframes
int i_max_steps = 180; //Número de pasos (tiempo)
...
typedef struct _frame
{
	...
	float rotRodDerInc;  //Incremento Rodilla Derecha
	float rotCabeza; //Cabeza
	float rotCabezaInc; //Incremento Cabeza
	float rotCuello; //Cuello
	float rotCuelloInc; //Incremento Cuello
	float rotBrazoIzq; //Brazo izquierdo
	float rotBrazoIzqInc; //Incremento Brazo Izquierdo
	float rotBrazoDer; //Brazo derecho
	float rotBrazoDerInc; //Incremento Brazo derecho
	float rotBrazoIzq2; //Brazo izquierdo
	float rotBrazoIzqInc2; //Incremento Brazo Izquierdo
	float rotBrazoDer2; //Brazo derecho
	float rotBrazoDerInc2; //Incremento Brazo derecho
...
void saveFrame(void) // Se manda a llamar cuando se oprime K
{
	...
	KeyFrame[FrameIndex].posX = posX; //Todas las variables se mandan aquí
	...
	KeyFrame[FrameIndex].rotRodDer = rotRodDer; ////// Rodilla Derecha 
	KeyFrame[FrameIndex].rotCabeza = rotCabeza; ////// Cabeza
	KeyFrame[FrameIndex].rotCuello = rotCuello; ////// Cuello
	KeyFrame[FrameIndex].rotBrazoIzq = rotBrazoIzq; ////// Brazo Izquierdo
	KeyFrame[FrameIndex].rotBrazoDer = rotBrazoDer; ////// Brazo derecho
	KeyFrame[FrameIndex].rotBrazoIzq2 = rotBrazoIzq2; ////// Brazo Izquierdo
	KeyFrame[FrameIndex].rotBrazoDer2 = rotBrazoDer2; ////// Brazo derecho
...
void resetElements(void) //Las variables se regresan a la posición actual
{
	...
	rotRodDer = KeyFrame[0].rotRodDer; ///////// Rodilla Derecha
	rotCabeza = KeyFrame[0].rotCabeza; ///////// Cabeza
	rotCuello = KeyFrame[0].rotCuello; ///////// Cuello
	rotBrazoIzq = KeyFrame[0].rotBrazoIzq; ///////// Brazo Izquierdo
	rotBrazoDer = KeyFrame[0].rotBrazoDer; ///////// Brazo Derecho
	rotBrazoIzq2 = KeyFrame[0].rotBrazoIzq2; ///////// Brazo Izquierdo
	rotBrazoDer2 = KeyFrame[0].rotBrazoDer2; ///////// Brazo Derecho
}
...
void interpolation(void) // Se calcula el incremento Keyframe siguiente - Keyframe actual / keyframes (tiempo)
{
	...
	KeyFrame[playIndex].rotRodDerInc = (KeyFrame[playIndex + 1].rotRodDer - KeyFrame[playIndex].rotRodDer) / i_max_steps; ////// Rodilla Derecha
	KeyFrame[playIndex].rotCabezaInc = (KeyFrame[playIndex + 1].rotCabeza - KeyFrame[playIndex].rotCabeza) / i_max_steps; ////// Cabeza
	KeyFrame[playIndex].rotCuelloInc = (KeyFrame[playIndex + 1].rotCuello - KeyFrame[playIndex].rotCuello) / i_max_steps; ////// Cuello
	KeyFrame[playIndex].rotBrazoIzqInc = (KeyFrame[playIndex + 1].rotBrazoIzq - KeyFrame[playIndex].rotBrazoIzq) / i_max_steps; ////// Brazo Izquierdo
	KeyFrame[playIndex].rotBrazoDerInc = (KeyFrame[playIndex + 1].rotBrazoDer - KeyFrame[playIndex].rotBrazoDer) / i_max_steps; ////// Brazo Derecho
	KeyFrame[playIndex].rotBrazoIzqInc2 = (KeyFrame[playIndex + 1].rotBrazoIzq2 - KeyFrame[playIndex].rotBrazoIzq2) / i_max_steps; ////// Brazo Izquierdo 2
	KeyFrame[playIndex].rotBrazoDerInc2 = (KeyFrame[playIndex + 1].rotBrazoDer2 - KeyFrame[playIndex].rotBrazoDer2) / i_max_steps; ////// Brazo Derecho 2
}
...
void monito()
{
	...
	glRotatef(15 + rotCuello, 1, 0, 0); ///////////Cuello
	fig7.cilindro(0.25, 0.25, 15, 0);
	glPushMatrix();//Cabeza
	glTranslatef(0, 1.0, 0);
	glRotatef(15 + rotCabeza, 0, 1, 0); ///////////Cabeza
	...
	glRotatef(15 + rotBrazoDer, 0, 1, 0); ///////////Brazo derecho
	glRotatef(15 + rotBrazoDer2, 1, 0, 0); ///////////Brazo derecho
	...
	glTranslatef(-1.25, 0.65, 0);
	glRotatef(15 + rotBrazoIzq, 0, 1, 0); ///////////Brazo izquierdo
	glRotatef(15 + rotBrazoIzq2, 1, 0, 0); ///////////Brazo izquierdo
	fig7.esfera(0.5, 12, 12, 0);
	glPushMatrix();
	glTranslatef(-0.25, 0, 0);
	glRotatef(45, 0, 1, 0);
	...
	glRotatef(-15, 1, 0, 0); // Pierna
	glTranslatef(0, -0.5, 0);
	fig7.prisma(1.0, 0.5, 1, 0);

	glPushMatrix();
	glTranslatef(0, -0.5, 0);
	glRotatef(15 + rotRodDer, 1, 0, 0); ///////////Rodilla Derecha
	...
	glPushMatrix();
	glTranslatef(0, -0.5, 0);
	glRotatef(15 + rotRodIzq, 1, 0, 0); //Posición +( Rotación  Variable ) que hace que se mueva la rodilla    
	glTranslatef(0, -0.75, 0);
	fig7.prisma(1.5, 0.5, 1, 0);
...
void InitGL(GLvoid)     // Inicializamos parametros
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotRodDerInc = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotRodDer = 0; ////// Se borra para volver a compilar e iniciar de nuevo

		KeyFrame[i].rotCabezaInc = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotCabeza = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotCuelloInc = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotCuello = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoIzqInc = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoIzq = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoDerInc = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoDer = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoIzqInc2 = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoIzq2 = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoDerInc2 = 0; ////// Se borra para volver a compilar e iniciar de nuevo
		KeyFrame[i].rotBrazoDer2 = 0; ////// Se borra para volver a compilar e iniciar de nuevo
...
void animacion()
{
	...
	if (play)
	{
		...
		{
			//Draw animation
			posX += KeyFrame[playIndex].incX; //Posición actual + incremento Keyframe (Para que se vea la interpolación)
			...
			rotRodDer += KeyFrame[playIndex].rotRodDerInc; ////////Rodilla Derecha
			rotCabeza += KeyFrame[playIndex].rotCabezaInc; ////////Cabeza
			rotCuello += KeyFrame[playIndex].rotCuelloInc; ////////Cuello
			rotBrazoIzq += KeyFrame[playIndex].rotBrazoIzqInc; ////////Brazo izquierdo
			rotBrazoDer += KeyFrame[playIndex].rotBrazoDerInc; ////////Brazo derecho
			rotBrazoIzq2 += KeyFrame[playIndex].rotBrazoIzqInc2; ////////Brazo izquierdo
			rotBrazoDer2 += KeyFrame[playIndex].rotBrazoDerInc2; ////////Brazo derecho
...
void keyboard(unsigned char key, int x, int y)  // Create Keyboard Function
{
	...
	case 'n':						/////// Tecla n para mover adelante la pierna del monito
		rotRodDer++;
		printf("%f \n", rotRodDer);
		break;

	case 'N':						////// Tecla N para mover atrás la pierna del monito
		rotRodDer--;
		printf("%f \n", rotRodDer);
		break;

	case 'q':						/////// Tecla q para mover arriba la cabeza del monito
		rotCabeza++;
		printf("%f \n", rotCabeza);
		break;

	case 'Q':						////// Tecla Q para mover abajo la cabeza del monito
		rotCabeza--;
		printf("%f \n", rotCabeza);
		break;

	case 'z':						/////// Tecla z para mover arriba la zabeza del monito
		rotCuello++;
		printf("%f \n", rotCuello);
		break;

	case 'Z':						////// Tecla Z para mover abajo la cabeza del monito
		rotCuello--;
		printf("%f \n", rotCuello);
		break;

	case 'x':						/////// Tecla x para mover arriba el brazo izquierdo del monito
		rotBrazoIzq++;
		printf("%f \n", rotBrazoIzq);
		break;

	case 'X':						////// Tecla X para mover abajo el brazo izquierdo del monito
		rotBrazoIzq--;
		printf("%f \n", rotBrazoIzq);
		break;

	case 'c':						/////// Tecla c para mover arriba el brazo derecho del monito
		rotBrazoDer++;
		printf("%f \n", rotBrazoDer);
		break;

	case 'C':						////// Tecla C para mover abajo el brazo derecho del monito
		rotBrazoDer--;
		printf("%f \n", rotBrazoDer);
		break;

	case 'r':						/////// Tecla r para mover arriba el brazo izquierdo del monito
		rotBrazoIzq2++;
		printf("%f \n", rotBrazoIzq2);
		break;

	case 'R':						////// Tecla R para mover abajo el brazo izquierdo del monito
		rotBrazoIzq2--;
		printf("%f \n", rotBrazoIzq2);
		break;

	case 't':						/////// Tecla t para mover arriba el brazo derecho del monito
		rotBrazoDer2++;
		printf("%f \n", rotBrazoDer2);
		break;

	case 'T':						////// Tecla T para mover abajo el brazo derecho del monito
		rotBrazoDer2--;
		printf("%f \n", rotBrazoDer2);
		break;
