...
Visual Studio 2017
Práctica 8 "Texturas"
Cubo.bmp con resolución 512x512 pixeles
*/
#include "texture.h"
...
void prisma (GLuint textura1, GLuint textura2)  //Funcion creacion prisma
{
	...
	GLfloat vertice [8][3] = {
		...
		glBindTexture(GL_TEXTURE_2D, textura2);   // 3
		glBegin(GL_POLYGON);	//Front
			glColor3f(1.0,1.0,1.0);
			glNormal3f( 0.0f, 0.0f, 1.0f);
			glTexCoord2f(0.25f, 0.33f); glVertex3fv(vertice[0]);
			glTexCoord2f(0.5f, 0.33f); glVertex3fv(vertice[4]);
		...
		glBindTexture(GL_TEXTURE_2D, textura1); // 1
		...
		glBindTexture(GL_TEXTURE_2D, textura1); // 4
		...
		glBindTexture(GL_TEXTURE_2D, textura1); // 5
		...
		glBindTexture(GL_TEXTURE_2D, textura1);   // 6
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(pos_camX, pos_camY, pos_camZ);
		...
		glPushMatrix(); // Colocar imagen de dado
			glColor3f(1.0, 1.0, 1.0); // Todos los colores correspondientes
			glScalef(1.0,1.0,1.0); // Proporción de 1 para la imagen Cubo.bmp
			prisma(t_Cubo.GLindex, t_Cubo.GLindex); // Se manda a llamar la función
		glPopMatrix(); // Termina colocación
